  ...
  public = list(
    ...
    execute = function(){
      ...
      if(is.vector(req$response)){
        ...
        if(startsWith(txt_resp, "<")) xmlObj = try(XML::xmlParse(txt_resp), silent = TRUE)
      }
      if(!is(xmlObj, "try-error")) if(is(xmlObj, "XMLInternalDocument")) if(!is.null(xmlNamespaces(xmlObj)$ows)){
